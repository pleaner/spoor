#!/usr/bin/env python3
# rate-card-sha256: b84adb214f0e37858df4ce223f78b93fa36451b86527a79a6735bd502ce16c85
"""Pricing script for Londolozi Private Granite Suites (londolozi).
...
Private Granite Suites (max two guests each — six guests total, the camp's
exclusive-use capacity), itemises the (3-night-capped) Guest Conservation
Contribution, and returns both RACK and STO (trade) figures with the chosen
configuration.

This mirrors the CANONICAL Londolozi pricing script (tree-camp-pricing.py) and
shares its conventions (see below).

Source rate cards (transcribed in data/raw/londolozi/private-granite-suites.md →
"## Rate card"; original PDFs: londolozi-rack-rates-2026.pdf and -2027.pdf, ZAR,
per person sharing per night, listed there as the "GRANITE CAMP" block).
...
PRIVATE GRANITE SUITES SPECIFICS / ASSUMPTIONS (surfaced for the assessment):
  * One room type, three suites, max occupancy 2 (the suites are couple suites;
    the card defines no third-person rate). Camp capacity is therefore six guests
    (the "exclusive use of six Guests" the brochure describes), so a party that
    cannot be seated within 3 suites × 2 (e.g. eight guests) is INFEASIBLE.
  * Adults-oriented: "no children under 16 years old"; children 6–16 only if the
    entire camp is booked exclusively, and all children pay the adult rate.
    Exclusive-use buyout is NOT assumed, so any guest under 16 makes the request
    INFEASIBLE (this is why the benchmark Family persona, which includes children
    6/10/14, is infeasible for Private Granite Suites).
"""
...
CURRENCY = "ZAR"
MIN_GUEST_AGE = 16        # no children under 16 (exclusive-use buyout not assumed)
LEVY_CAP_NIGHTS = 3       # Guest Conservation Contribution capped at 3 nights
LEVY_NAME = "Guest Conservation Contribution"

# Per-night rate cards. (pps = per person sharing; single = single occupancy incl.
# the 25% single supplement; levy = Guest Conservation Contribution pppn.)
CARD_2026 = {"pps": 54000.0, "single": 67500.00, "levy": 957.50}   # -2026.pdf
CARD_2027 = {"pps": 59950.0, "single": 74937.50, "levy": 1015.00}  # -2027.pdf
BOUNDARY = datetime.date(2026, 12, 15)  # nights on/before → 2026 card, after → 2027

SUITES = {"Private Granite Suite": {"inventory": 3, "max": 2}}
...
def room_cost(card, suite, ages):
    ...
    STO == RACK (no trade rate published), so both elements are equal.
    """
    n = len(ages)
    if any(a < MIN_GUEST_AGE for a in ages):
        return None  # no children under 16 (non-exclusive use)
    if suite == "Private Granite Suite":
        if n == 1:
            return (card["single"], card["single"], ["single occupancy (25% supplement)"])
        if n == 2:
            return (2 * card["pps"], 2 * card["pps"], ["pppn sharing ×2"])
        return None  # max occupancy 2; no third-person rate
    return None
...
_ASSUMPTIONS = [
    ...
    "Private Granite Suites take no guests under 16 unless the camp is booked "
    "exclusively; exclusive-use buyout is not assumed, so under-16 parties are "
    "infeasible (children 6–16 would pay the adult rate under buyout only).",
    "Private Granite Suite max occupancy 2 (couple suite; no third-person rate on "
    "the card); three suites give a camp capacity of six guests, so parties that "
    "cannot be seated within 3 suites × 2 are infeasible.",
...
def price(start, end, ages=None):
    ...
    if any(a < MIN_GUEST_AGE for a in ages):
        return _infeasible(start, end, ages,
                           f"Private Granite Suites does not accept guests under "
                           f"{MIN_GUEST_AGE} (non-exclusive use)")
    ...
    for d in dates:
        ...
        if best is None:
            return _infeasible(start, end, ages,
                               ...
                               f"camp capacity is six guests (3 suites × 2)")
        rack_total += best[0]
        sto_total += best[1]
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Price a stay at Londolozi Private Granite Suites.")
