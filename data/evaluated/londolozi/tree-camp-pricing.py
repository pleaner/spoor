#!/usr/bin/env python3
# rate-card-sha256: d531620c1d77688a9a932339111382d447c161f4bd8813bd2dd75ad0fd80276d
"""Pricing script for Londolozi Tree Camp (londolozi).

Self-contained and stdlib-only. Turns the property's raw rate card into:

    price(start, end, ages=[...]) -> dict

It brute-forces the cheapest valid way to seat a party across the camp's six
Tree Camp Suites, itemises the (3-night-capped) Guest Conservation Contribution,
and returns both RACK and STO (trade) figures with the chosen configuration.

This is the CANONICAL Londolozi pricing script — the four other camps in the
group share its conventions (see below) and mirror this structure.

Source rate cards (transcribed in data/raw/londolozi/tree-camp.md → "## Rate
card"; original PDFs: londolozi-rack-rates-2026.pdf and -2027.pdf, ZAR, per
person sharing per night, "TREE CAMP" block).
...
TREE CAMP SPECIFICS / ASSUMPTIONS (surfaced for the completeness assessment):
  * One room type, six suites, max occupancy 2 (the suites are couple suites; the
    card defines no third-person rate).
  * Adults-oriented: "no children under 16 years old" unless the whole camp is
    booked exclusively. Exclusive-use buyout is NOT assumed, so any guest under 16
    makes the request INFEASIBLE (this is why the benchmark Family persona, which
    includes children 6/10/14, is infeasible for Tree Camp).
"""
...
CARD_2026 = {"pps": 48450.0, "single": 60562.50, "levy": 957.50}   # -2026.pdf
CARD_2027 = {"pps": 54750.0, "single": 68437.50, "levy": 1015.00}  # -2027.pdf
BOUNDARY = datetime.date(2026, 12, 15)  # nights on/before → 2026 card, after → 2027

SUITES = {"Tree Camp Suite": {"inventory": 6, "max": 2}}
...
_ASSUMPTIONS = [
    ...
    "Tree Camp takes no guests under 16 unless the camp is booked exclusively; "
    "exclusive-use buyout is not assumed, so under-16 parties are infeasible.",
    "Tree Camp Suite max occupancy 2 (couple suite; no third-person rate on the card).",
...
def price(start, end, ages=None):
    ...
    if any(a < MIN_GUEST_AGE for a in ages):
        return _infeasible(start, end, ages,
                           f"Tree Camp does not accept guests under {MIN_GUEST_AGE} "
                           "(non-exclusive use)")
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Price a stay at Londolozi Tree Camp.")
